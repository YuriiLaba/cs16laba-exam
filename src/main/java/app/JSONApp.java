...
public class JSONApp {
    ...
    private static void print(Json json) {
        ...
        public static JsonObject sessionResult() {
            ...
            JsonPair name = new JsonPair("name", new JsonString("Andrii"));
            JsonPair surname = new JsonPair("surname", new JsonString("Rodionov"));
            Json jsonNumber = new JsonNumber(2);
            JsonPair jsonPair = new JsonPair("year", jsonNumber);
            JsonObject jsonObject = new JsonObject(new JsonPair("course", new JsonString("OOP")),
                    new JsonPair("mark", new JsonNumber(3)), new JsonPair("passed", new JsonBoolean(true)));

            JsonObject jsonObject1 = new JsonObject(new JsonPair("course", new JsonString("English")),
                    new JsonPair("mark", new JsonNumber(5)), new JsonPair("passed", new JsonBoolean(true)));

            JsonObject jsonObject2 = new JsonObject(new JsonPair("course", new JsonString("Math")),
                    new JsonPair("mark", new JsonNumber(2)), new JsonPair("passed", new JsonBoolean(false)));
            JsonArray jsonArray = new JsonArray(jsonObject, jsonObject1, jsonObject2);
            JsonPair jsonPair1 = new JsonPair("exams", jsonArray);
            return new JsonObject(name, surname, jsonPair, jsonPair1);
        }
    }
